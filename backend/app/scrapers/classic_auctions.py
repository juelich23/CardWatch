#!/usr/bin/env python3
"""
Classic Auctions Scraper
Fetches auction items from Classic Auctions (hockey focused).
Uses Playwright with non-headless mode due to Akamai WAF protection.
"""
...
class ClassicAuctionsScraper:
    def __init__(self):
        self.base_url = "https://classicauctions.net"
        self.catalog_url = f"{self.base_url}/catalog.aspx"
        self.auction_house_name = "classic_auctions"
        self._browser = None
    ...
    def extract_category(self, title: str) -> Optional[str]:
        ...
        # Classic Auctions is hockey-focused
        if any(kw in title_upper for kw in ['HOCKEY', 'NHL', 'STANLEY CUP', 'CANADIENS', 'MAPLE LEAFS', 'BRUINS', 'RANGERS', 'FLYERS']):
            return 'Hockey'
        ...
        if any(kw in title_upper for kw in ['FOOTBALL', 'NFL']):
            return 'Football'
        if any(kw in title_upper for kw in ['BASEBALL', 'MLB']):
            return 'Baseball'

        return 'Hockey'  # Default for Classic Auctions
    ...
    async def _ensure_browser(self):
        """Ensure Playwright browser is initialized (non-headless for WAF bypass)"""
        if self._browser is None:
            self._playwright = await async_playwright().start()
            self._browser = await self._playwright.chromium.launch(
                headless=False,  # Required to bypass Akamai WAF
                channel="chrome"
            )
    ...
    def get_pagination_info(self, soup: BeautifulSoup) -> dict:
        ...
        # Look for pagination elements - Classic Auctions uses page=N parameter
    ...
    async def scrape(self, db: AsyncSession, max_items: int = 1000, max_pages: int = 50) -> list:
        ...
        print("🔍 Fetching items from Classic Auctions...")

        all_items = []

        try:
            # Fetch first page
            print("📡 Fetching catalog page (using non-headless Playwright)...")
        ...
        auction_external_id = "classic-current"
    ...
    async def health_check(self) -> HealthCheckResult:
        """Check if Classic Auctions website is reachable"""
        try:
            html = await self.fetch_page(self.catalog_url)
            soup = BeautifulSoup(html, 'html.parser')
            items = soup.find_all('div', class_='lot')
            ...
            if items:
                return HealthCheckResult(
                    healthy=True,
                    message="Classic Auctions is reachable",
                    details={"items_on_page": len(items)}
                )
            return HealthCheckResult(
                healthy=False,
                message="Classic Auctions returned no items",
                details={"html_length": len(html)}
            )
        except Exception as e:
            return HealthCheckResult(
                healthy=False,
                message=f"Classic Auctions unreachable: {str(e)}",
                details={"error": str(e)}
            )
        finally:
            await self._close_browser()
...
async def main():
    ...
    scraper = ClassicAuctionsScraper()
