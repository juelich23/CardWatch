#!/usr/bin/env python3
"""
Hunt Auctions Scraper
Fetches auction items from Hunt Auctions.
Uses Playwright for browser-based scraping.
Note: Next auction is Jan 2026, so may have no current items.
"""
...
class HuntAuctionsScraper:
    def __init__(self):
        self.base_url = "https://huntauctions.com"
        self.auction_house_name = "hunt"
        self._browser = None
    ...
    def extract_category(self, title: str) -> Optional[str]:
        ...
        if any(kw in title_upper for kw in ['HOCKEY', 'NHL']):
            return 'Hockey'
        if any(kw in title_upper for kw in ['MEMORABILIA', 'JERSEY', 'SIGNED', 'AUTOGRAPH']):
            return 'Memorabilia'
    ...
    def parse_items(self, html: str) -> list:
        ...
        lots = soup.find_all('div', class_='lot') or \
               ...
               soup.find_all('article', class_='lot')
        ...
        for lot_div in lots:
            try:
                ...
                current_match = re.search(r'Current\s*Bid[:\s]*\$?([\d,]+)', text, re.I)
                if current_match:
                    current_bid = self.parse_price(current_match.group(1))

                start_match = re.search(r'(?:Start|Min|Opening)\s*Bid[:\s]*\$?([\d,]+)', text, re.I)
                if start_match:
                    starting_bid = self.parse_price(start_match.group(1))
    ...
    async def scrape(self, db: AsyncSession, max_items: int = 1000, max_pages: int = 50) -> list:
        ...
        print("Fetching items from Hunt Auctions...")
        ...
        try:
            ...
            catalog_urls = [
                ...
                f"{self.base_url}/current-auction",
        ...
        if not all_items:
            print("   No items found - next auction may be scheduled for later (Jan 2026)")
            return []

        # Create or update auction
        auction_external_id = "hunt-current"
    ...
    async def health_check(self) -> HealthCheckResult:
        """Check if Hunt Auctions is reachable"""
        try:
            html = await self.fetch_page(self.base_url)
            if len(html) > 1000:
                return HealthCheckResult(
                    healthy=True,
                    message="Hunt Auctions is reachable",
                    details={"html_length": len(html)}
                )
            return HealthCheckResult(
                healthy=False,
                message="Hunt Auctions returned minimal content",
                details={"html_length": len(html)}
            )
        except Exception as e:
            return HealthCheckResult(
                healthy=False,
                message=f"Hunt Auctions unreachable: {str(e)}",
                details={"error": str(e)}
            )
        finally:
            await self._close_browser()
