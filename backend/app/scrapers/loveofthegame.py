#!/usr/bin/env python3
"""
Love of the Game Auctions Scraper
Fetches auction items from Love of the Game Auctions.
...
class LoveOfTheGameScraper:
    def __init__(self):
        self.base_url = "https://loveofthegameauctions.com"
        self.auction_house_name = "loveofthegame"
        self._browser = None
    ...
    async def scrape(self, db: AsyncSession, max_items: int = 1000, max_pages: int = 50) -> list:
        ...
        print("Fetching items from Love of the Game Auctions...")
        ...
        auction_external_id = "loveofthegame-current"
        ...
        if not auction:
            auction = Auction(
                ...
                title="Love of the Game Auctions",
                status="active"
            )
            db.add(auction)
            await db.flush()
    ...
    async def health_check(self) -> HealthCheckResult:
        """Check if Love of the Game Auctions is reachable"""
        try:
            html = await self.fetch_page(self.base_url)
            if len(html) > 1000:
                return HealthCheckResult(
                    healthy=True,
                    message="Love of the Game Auctions is reachable",
                    details={"html_length": len(html)}
                )
            return HealthCheckResult(
                healthy=False,
                message="Love of the Game Auctions returned minimal content",
                details={"html_length": len(html)}
            )
        except Exception as e:
            return HealthCheckResult(
                healthy=False,
                message=f"Love of the Game Auctions unreachable: {str(e)}",
                details={"error": str(e)}
            )
        finally:
            await self._close_browser()
