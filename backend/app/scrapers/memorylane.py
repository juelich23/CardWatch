#!/usr/bin/env python3
"""
Memory Lane Inc Scraper
Fetches auction items from Memory Lane Inc by parsing HTML pages.
...
class MemoryLaneScraper:
    def __init__(self):
        self.base_url = "https://bid.memorylaneinc.com"
        self.gallery_url = f"{self.base_url}/Lots/Gallery"
        self.auction_house_name = "memorylane"
        self._browser = None
    ...
    async def scrape(self, db: AsyncSession, max_items: int = 1000, max_pages: int = 50) -> list:
        ...
        print("🔍 Fetching items from Memory Lane Inc...")
        ...
        auction_external_id = "memorylane-current"
    ...
    async def health_check(self) -> HealthCheckResult:
        """Check if Memory Lane website is reachable (using Playwright)"""
        try:
            html = await self.fetch_page(self.gallery_url)
            soup = BeautifulSoup(html, 'html.parser')
            items = soup.find_all('div', class_='item')
            ...
            if items:
                return HealthCheckResult(
                    healthy=True,
                    message="Memory Lane Inc is reachable",
                    details={"items_on_page": len(items)}
                )
            return HealthCheckResult(
                healthy=False,
                message="Memory Lane returned no items",
                details={"html_length": len(html)}
            )
        except Exception as e:
            return HealthCheckResult(
                healthy=False,
                message=f"Memory Lane unreachable: {str(e)}",
                details={"error": str(e)}
            )
        finally:
            await self._close_browser()
